...
async def run_script(commands: [int], script: [str]):
    input = encode(script)
    output = []
    await Program(commands, input, output).execute()
    print(output[-1])

async def walk(commands: [int]):
    script = ['NOT A T', 'NOT B J', 'OR T J', 'NOT C T', 'OR T J', 'AND D J', 'WALK']
    await run_script(commands, script)

async def run(commands: [int]):
    script = ['NOT A T', 'NOT B J', 'OR T J', 'NOT C T', 'AND H T', 'OR T J', 'AND D J', 'RUN']
    await run_script(commands, script)

commands = [int(token) for token in open(argv[1]).read().strip().split(',')]
aio.run(walk(commands))
aio.run(run(commands))